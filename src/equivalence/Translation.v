...
Fixpoint translate (M : object_model) (E : tran_env) (t : tm) : option (rex_or_rel * list var_name * T_e * rel) := 
    ...
    *)
    | CLit v => 
        match v with
          |  Ib_Bool b => Some (Rex (RLit (Ira_Bool b)),    [], Te_Single (Th_Basic Tb_Bool),   RAEmpty)
          |  Ib_Int z => Some (Rex (RLit (Ira_Int z)),      [], Te_Single (Th_Basic Tb_Int),    RAEmpty)
          |  Ib_Real r => Some (Rex (RLit (Ira_Real r)),    [], Te_Single (Th_Basic Tb_Real),   RAEmpty)
          |  Ib_String s => Some (Rex (RLit (Ira_String s)),[], Te_Single (Th_Basic Tb_String), RAEmpty)
        end
    ...
    *)
    | CUnop op t =>
        ...
        (* rex *)
        | Some (Rex e1, [], te, RAEmpty) =>
            match unop_type op te with 
            | Some te' => 
                Some (Rex (RUnop op e1), [], te', RAEmpty)
            | _ => None
            end
        (* rel *)
        | Some (Rel rel, vl, te, RAEmpty) =>
            match unop_type op te with 
            | Some te' => 
    ...
    *)
    ...
    | CBinop op t1 t2 =>
        ...
        *)
        | Some (Rex e1, [], te1, RAEmpty), Some (Rex e2, [], te2, RAEmpty) =>
            match binop_type op te1 te2 with 
            | Some te' =>
                Some (Rex (RBinop op e1 e2), [], te', RAEmpty)
            | _ => None
            end
        (* rex 与 rel 
            转换规则:
            ...
            - 若t1为rel，t2为rex
                ...
                - 类型由OCLTyping中binop_type函数计算得到
        *)
        | Some (Rex e1, [], te1, RAEmpty), Some (Rel rel2, vl2, te2, RAEmpty) =>
            match binop_type op te1 te2 with 
            | Some te' =>
        ...
        (* rel 与 rex *)
        | Some (Rel rel1, vl1, te1, RAEmpty), Some (Rex e2, [], te2, RAEmpty) =>
            match binop_type op te1 te2 with 
            | Some te' =>
    ...
    *)
    | CAttr tm attr =>
        ...
        | Some (Rel rel, vl, Te_Single (Th_Object cn), RAEmpty ) =>
    ...
    | CUnion t1 t2 =>
        ...
        | Some (Rel rel1, vl1, te1, dim_rel1), Some (Rel rel2, vl2, te2, dim_rel2) =>
            ...
            *)
            ...
            (* 
                ...
                - 若rel1依赖变量列表不为空，rel2只有一列val_col列
                    ...
                    - 类型为te1 （ 若为有效的语句，te1=te2）
            *)
                let rel2' := RACartesian dim_rel1 rel2 in
                Some (
                    Rel (RAUnion rel1 rel2'),
                    vl1,
                    te1,
            ...
            (* 
                ...
                - 若rel1只有一列val_col列，rel2依赖变量列表不为空
                    ...
                    - 类型为te1 （ 若为有效的语句，te1=te2）
            *)
                let rel1' := RACartesian dim_rel2 rel1 in
                Some (
                    Rel (RAUnion rel1' rel2),
                    vl2,
                    te1,
            ...
            *)
                ...
                | [] => 
                    (* 列顺序乱了 *)
                    let rel1' := RACartesian rel1 dim_rel2 in
                    let rel2' := RACartesian dim_rel1 rel2 in 
                    ...
                    Some (
                        ...
                        unioncols,
                        te1,
                        RAProject (proj_cols unioncols) (RACartesian dim_rel1 dim_rel2)
                    )
                ...
                | x :: xs => 
                    ...
                    | Some jcond =>
                        ...
                        let rel1' := RAJoin jcond rel1 (RAProject (proj_cols recols2) dim_rel2) in
                        let rel2' := RAJoin jcond (RAProject (proj_cols recols1) dim_rel1) rel2 in
                        (* 使用project调整输出顺序 *)
    ...
    *)
    ...
    (* 
        ...
        - 转换为按照依赖变量列表进行分组聚合，对val_col列进行聚合（相同依赖变量的行对应的val_col的值组成一个bag）
            - 若依赖变量列表为空，表示全局聚合，得到一个标量值（暂不支持，TODO）
    ...
    *)
    | CAggregate aggop tm =>
        ...
        | Some (Rel rel, vl, Te_Bag th, dim_rel ) =>
            ...
            |  x :: xs, Some th' =>
                ...
                (* vl中的依赖变量 与 dim_rel中的列应该相同，即事实表rel比维度表dim_rel多一列val_col *)
                | AggSize =>
                    let nonzero_rel := RAAggregate vl [(val_col, aggop, val_col)] rel in
                    let zero_rel := RADiff dim_rel (RADistinct (RAProject (proj_cols vl) rel)) in
                    let ex_zero_rel := RAProject (List.app (proj_cols vl) [mkProj val_col (RLit (Ira_Int 0))]) zero_rel in
                    Some (
                        Rel (RAUnion nonzero_rel ex_zero_rel),
                        vl,
                        Te_Single (Th_Basic Tb_Int),
                        dim_rel
                    )
                
                | _ =>
                (* AggSum, AggMin, AggMax操作在空bag上无定义，OCL会求值失败*)
                    Some (
    ...
    (* 
        ...
        - 转换为对
            ...
            - 选择出val_col为true的行，投影出vl1的所有列，以及将var列投影为val_col列（！！此时会丢失空bag的组。size聚合操作可能会受影响，不会出现size=0的组，TODO）
        - 依赖变量列表与原rel一致
        - 表达式的类型为原rel的类型
    *)
    | CSelect t1 var t2 =>
