...
**Table N1.  Table of main notations of Syntax of Object Models**

| num | Notation                       | Description                                                                                                                                                           |
| --- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | $NAME$                         | A set of finite, non-empty names $NAME \subseteq A^+$ over alphabet $A$.                                                                                              |
| 2   | $\Sigma=(\mathcal{T}, \Omega)$ | A set of type names, and $\Omega$ being a set of operations over types in $\mathcal{T}$.                                                                              |
| 3   | $CLASS$                        | A finite set of names $CLASS \subseteq NAME$.                                                                                                                         |
| 4   | $t_c$                          | An object type $t_c \in \mathcal{T}$ having the same name as the class $c \in CLASS$.                                                                                 |
| 5   | $ATT_c$                        | A set of signatures $a:t_c \rightarrow t$ where $t \in T$ be a type, the attribute name $a$ is an element of $NAME$, and $t_c \in T$ is the object type of class $c$. |
| 6   | $ASSOC$                        | A finite set of association names $ASSOC \in NAME$.                                                                                                                   |
| 7   | $FD_c$                         | The full descriptor of a class $c \in CLASS$ containing all attributes and navigable role names defined for the class.                                                |




**Table N2.  Table of main notations of Interpretation of Object Models**

| num | Notation            | Description                                                                                                                                                                                                               |
| --- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | $I_{CLASS}(c)$      | The domain of a class $c \in CLASS$. $I_{CLASS}(c) = oid(c)$, where $oid(c) = \{ c_1 , c_2, ...\}$ is an infinite set of object identifiers of a class $c$.                                                               |
| 2   | $I_{ASSOC}(as)$     | The domain of a association $as \in ASSOC$. $I_{ASSOC}(as) = I_{CLASS}(c_1) \times I_{CLASS}(c_2)$                                                                                                                        |
| 3   | $I(t)$              | Value domain of type $t$.                                                                                                                                                                                                 |
| 4   | $\sigma(M)$         | A system state for a model $M$. $\sigma(M) = (\sigma_{CLASS}, \sigma_{ATT}, \sigma_{ASSOC})$.                                                                                                                             |
| 5   | $\sigma_{CLASS}(c)$ | The finite sets $\sigma_{CLASS}(c)$ contain all objects of a class $c \in CLASS$ existing in the system state: $\sigma_{CLASS}(c) \subset oid(c)$.                                                                        |
| 6   | $\sigma_{ATT}$      | Functions $\sigma_{ATT}$ assign attribute values to each object: $\sigma_{ATT}(a) : \sigma_{CLASS}(c) \rightarrow I(t)$ for each $a:t_c \rightarrow t \in ATT_c$ .                                                        |
| 7   | $\sigma_{ASSOC}$    | The finite sets $\sigma_{ASSOC}$ contain links connecting objects. For each $as \in ASSOC: \sigma_{ASSOC}(as) \subset I_{ASSOC}(as)$. A link set must satisfy all multiplicity specifications defined for an association. |





**Table N3.  Table of main notations of OCL Types and Operations**

| num | Notation                                                                                               | Description                                                                                                                                                                                                                                                                    |
| --- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | $\Sigma = (T, \Omega)$                                                                                 | A data signature $\Sigma = (T, \Omega)$ to describe the types and operations, where $T$ is defined by a mapping that assigns each type a domain, $\Omega$ is defined by a mapping that assigns each operation a function.                                                      |
| 1   | $I(t)$                                                                                                 | Value domain of type $t$. where $I(Integer) = \mathbb{Z}$, $I(Real) = \mathbb{R}$, $I(Boolean) = \{ true, false\}$, $I(String) = A^*$.                                                                                                                                         |
| 2   | $\Sigma_B = (T_B, \Omega_B)$                                                                           | Basic types and their operations.                                                                                                                                                                                                                                              |
| 3   | $12 : \rightarrow Integer$                                                                             | A set of operations in $\Omega_B$ that do not have parameters.                                                                                                                                                                                                                 |
| 4   | $I(\omega: t_1 \times \cdot \cdot \cdot \times t_n \rightarrow t)$                                     | The semantics of an operation with signature $\omega : t_1 \times \cdot \cdot \cdot \times t_n \rightarrow t $, which is a total function $I(\omega: t_1 \times \cdot \cdot \cdot \times t_n \rightarrow t) : I(t_1) \times \cdot \cdot \cdot \times I(t_n) \rightarrow I(t)$. |
| 5   | $\Sigma_C = (T_C, \Omega_C)$                                                                           | Object types and their operations                                                                                                                                                                                                                                              |
| 6   | $I_{ATT}(a : t_c \rightarrow t)$                                                                       | The semantics of attribute signature $a : t_c \rightarrow t$, which is an attribute value function $I_{ATT}(a : t_c \rightarrow t) : I(t_c) \rightarrow I(t)$ mapping objects of class $c$ to a value of type $t$.                                                             |
| 7   | $I(t_{j(as,r_i)}:t_{c_i} \rightarrow t_{c_j})$ or $ I(t_{j(as,r_i)}:t_{c_i} \rightarrow Set(t_{c_j}))$ | The semantics of navigation operations.                                                                                                                                                                                                                                        |
...
An object model has the following components :
• a set of classes
• a set of attributes for each class
• a set of associations with role names and multiplicities
**(不考虑 class operation，generalization)**



- $X^+$ 表示集合$X$的正闭包（positive closure）, $X^+$是由 $X$ 中元素构成的、长度至少为 1 的有限序列的集合。$A^{+} \triangleq \bigcup_{n=1}^{\infty} A^{n}$ 
- $P(X)$ 表示集合$X$的幂集（power set）运算,P(X) 是$X$的所有子集所构成的集合。$P(X)\triangleq \{S∣S\subseteq X\}$
- $N$ 表示自然数，$N^+$表示正自然数。
...
A class provides a common description for a set of objects sharing the same properties.

> **Definition A.1 (Classes)**
> The set of classes is a finite set of names $CLASS \subseteq NAME$.

Each class $c \in CLASS$ induces an $\textit{object type}$ $t_c \in \mathcal{T}$ having the same name as the class. A value of an object type refers to an object of the corresponding class.



#### A.1.1.3 Attributes

Objects are associated with attribute values describing properties of the object. An attribute has a name and a type specifying the domain of attribute values.


> **Definition A.2 (Attributes)**
> Let $t_b \in T_B$ be a basic type. The attributes of a class $c \in CLASS$ are defined as a set $ATT_c$ of signatures $a:t_c \rightarrow t_b$ where the attribute name $a$ is an element of $NAME$, and $t_c \in T$ is the type of class $c$.



(Attribute naming constraint) All attributes of a class have distinct names. In particular, an attribute name may not be used again to define another attribute with a different type.
$ \forall t_b, t_b' \in T_B: (a: t_c \rightarrow t_b \in ATT_c \land a:t_c \rightarrow t_b' \in ATT_c) \Rightarrow t_b=t_b'$

#### A.1.1.5 Associations
Associations describe structural relationships between classes. Generally, classes may participate in any number of associations, and we only consider associations between two classes. For many problems the use of binary associations is often sufficient. **(不考虑n-ary关系和self-association)**

> **Definition A.4 (Associations)**
> The set of associations is given by
> i. a finite set of names $ASSOC \in NAME$,
> ii. a function associates: $$
associates:
\begin{cases}
ASSOC \rightarrow CLASS \times CLASS \\
...
The function $associates$ maps each association name $as \in ASSOC$ to a two elements list $\langle c_1, c_2 \rangle$ of classes participating in the association. 


##### Role Names

We assign each class participating in an association a unique role name. (一个Association的所有role需要distinct；一个class涉及的所有role需要distinct)
...
roles :
\begin{cases}
ASSOC \rightarrow NAME \times NAME \\
...
navends :
\begin{cases}
CLASS \times ASSOC  \rightarrow P(NAME) \\
...
An association specifies the possible existence of links between objects of associated classes. The number of links that an object can be part of is specified with multiplicities. A multiplicity specification in UML can be represented by a set of natural numbers $N^+=\{1,2,...\}$.
...
The UML standard requires that properties of a full descriptor must be distinct. Attribute names and role names must not conflict. This is necessary because in OCL the same notation is used for attribute access and navigation by role name. For example, the expression self.x may either be a reference to an attribute x or a reference to a role name x. (一个class中的attr和role的名字必须distinct)

$$\forall (a:t_c \rightarrow t_b \in ATT_c) : \forall r \in navends(c) :: (a \ne r) $$
...
> ii. $ATT_c$ is a set of operation signatures for functions mapping an object of class $c \in CLASS$ to an associated attribute value (Definition A.2).
...
Objects, links, and attribute values constitute the state of a system at a particular moment in time. A system is in different states as it changes over time. Therefore, a system state is also called a snapshot of a running system. With respect to OCL, we can in many cases concentrate on a single system state given at a discrete point in time. For example, a system state provides the complete context for the evaluation of OCL invariants.



> **Definition A.12 (System State)**
> A system state for a model $M$ is a structure $\sigma(M) = (\sigma_{CLASS}, \sigma_{ATT}, \sigma_{ASSOC})$.
> i. The finite sets $\sigma_{CLASS}(c)$ contain all objects of a class $c \in CLASS$ existing in the system state: $\sigma_{CLASS}(c) \subset oid(c)$.
> ii. Functions $\sigma_{ATT}$ assign attribute values to each object: $\sigma_{ATT}(a) : \sigma_{CLASS}(c) \rightarrow I(t_b)$ for each $a:t_c \rightarrow t_b \in ATT_c$ .
...
- $X^*$表示 Kleene 闭包（Kleene star），也称为 星闭包。$X^∗ \triangleq \bigcup_{n=0}^{\infty} X^{n}$.
...
**Table A.1 - - Schema for operations on basic types(TODO)**
...
> The semantics of an operation with signature $\omega : t_1 \times \cdot \cdot \cdot \times t_n \rightarrow t $ is a total function $I(\omega: t_1 \times \cdot \cdot \cdot \times t_n \rightarrow t) : I(t_1) \times \cdot \cdot \cdot \times I(t_n) \rightarrow I(t)$.
...
We can define the semantics of the other operations in Table A.1 analogously. Table A.2 shows the interpretation of Boolean operations. Since the semantics of the other basic operations for Integer, Real, and String values is rather obvious, we will not further elaborate on them here.
...
The syntax of object types and their operations is defined by a signature $\Sigma_C = (T_C, \Omega_C)$. $T_C$ is the set of object types, and $\Omega_C$ is the set of signatures describing operations on object types.


> **Definition A.19 (Syntax Of Object Types)**
> Let $OM$ be a object model with a set $CLASS$ of class names. The set $T_C$ of object types is defined such that for each class $c \in CLASS$ there is a type $t \in T_C$ having the same name as the class $c$.
...
Attribute operations are declared in a model specification by the set $ATT_c$ for each class $c$. The set contains signatures $a : t_c \rightarrow t_b$ with a being the name of an attribute defined in the class $c$. The type of the attribute is $t_b \in T_b$. All attribute operations in $ATT_c$ are elements of $\Omega_C$. The semantics of an attribute operation is a function mapping an object identifier to a value of the attribute domain. An attribute value depends on the current system state.


> **Definition A.21 (Semantics of Attribute Operations)**
> An attribute signature $a : t_c \rightarrow t_b$ in $\Omega_C$ is interpreted by an attribute value function $I_{ATT}(a : t_c \rightarrow t_b) : I(t_c) \rightarrow I(t_b)$ mapping objects of class $c$ to a value of type $t_b$.
$$
I_{ATT}(a : t_c \rightarrow t_b)(c)
\begin{cases}
\Omega_{ATT}(a)(c)， \qquad  \ c \in \Omega_{CLASS}(c) 
\end{cases}
$$
...
> i. $if \ M_j - \{1\} = \emptyset  \ then \  r_{j(as;r_i)} : t_{c_i} \rightarrow t_{c_j} \in \Omega_{nav}(c) $.
> ii. $if \ M_j - \{1\} \ne \emptyset  \ then \  r_{j(as;r_i)} : t_{c_i} \rightarrow Bag(t_{c_j}) \in \Omega_{nav}(c) $.

The result type of a navigation over binary associations is the type of the target class if the multiplicity of the target is given as 1 (i). All other multiplicities allow an object of the source class to be linked with multiple objects of the target class. Therefore, we need a set type to represent the navigation result.
...
> $$ L(as)(c_i) = \{ c_j | \langle c_i,c_j \rangle \in \sigma_{ASSOC}(as) \} $$
> The semantics of operations in $\Omega_{nav}(c)$ is then defined as
i. $I(t_{j(as,r_i)}:t_{c_i} \rightarrow t_{c_j})(c_i)=c_j$,
ii. $ I(t_{j(as,r_i)}:t_{c_i} \rightarrow Bag(t_{c_j}))(c_i) = L(as)(c_i) $.



#### A.2.5 Collection Types

We call a type that allows the aggregation of several values into a single value a complex type. OCL provides the complex types $Bag(t)$ for describing collections of values of type $t$. The $Bag(t)$ types are homogeneous in the sense that all elements of a collection must be of the same type $t$.
...
> Let $\hat{T}$ be a set of types and $l_1, \cdot \cdot \cdot , l_n \in NAME$ a set of disjoint names. The set of type expressions $T_{Expr}(\hat{T})$ over $\hat{T}$ is defined as follows.
i. If $t \in \hat{T}$ then $ t\in T_{Expr}( \hat{T})$.
ii. if $t \in \hat{T}$ then $Bag(t) \in T_{Expr}(\hat{T}) $.


The definition says that every type $t \in \hat{T}$ can be used as an element type for constructing a set or bag type.

For the definition of the semantics of type expressions we make the following conventions. Let $F(S)$ denote the set of all finite subsets of a given set $S$, and $B(S)$ is the set of all finite multisets (bags) over $S$.
...
Operations for constructing collection values by enumerating their element values are called constructors. For types $t \in T_{Expr}(\hat{T}) $ constructors in $\Omega_{T_{Expr}(\hat{T})}$ are defined below. A parameter list $t \times \cdot \cdot \cdot \times t$ denotes $n (n \ge 0)$ parameters of the same type $t$. We define constructors $mkBag_t$ not only for any type $t$ but also for any finite number $n$ of parameters.
...
The definition of operations of collection types comprises the set of all predefined collection operations. Operations common to the types $Bag(t)$ is defined for the supertype $Collection(t)$. Table A.6 shows the operation schema for these operations. For all $t \in T_{Expr}( \hat{T})$, the signatures resulting from instantiating the schema are included in $\Omega_{T_{Expr}}( \hat{T})$ . The right column of the table illustrates the intended set-theoretic interpretation. For this purpose, $C$, $C_1$, $C_2$ are values of type $Collection(t)$, and $v$ is a value of type $t$.


**Table A.6 - - Operations for type Collection(t) (TODO)**
...
The operation schema in Table A.6 can be applied to bags by substituting Bag(t) for all occurrences of type Collection(t). A semantics for the operations in Table A.6 can be easily defined for each of the concrete collection types Bag(t). The semantics for the operations of Collection(t) can then be reduced to the concrete types because every collection type is a bag. Consider, for example, the operation count : $Bag(t) \times t \rightarrow Integer$ that counts the number of occurrences of an element $v$ in a set $s$. The semantics of count is:
<!-- $$
...
**Table A.7 - Operations for type $Set(t)$(TODO)**
|                      | Signature                                  | Semantics                       |
| -------------------- | ------------------------------------------ | ------------------------------- |
...
**Table A.8 - Operations for type $Bag(t)$(TODO)**
|                      | Signature                                  | Semantics                       |
| -------------------- | ------------------------------------------ | ------------------------------- |
| union:               | $ Bag(t) \times Bag(t) \rightarrow Bag(t)$ | $B_1 \cup B_2$                  |
| intersection:        | $ Bag(t) \times Bag(t) \rightarrow Bag(t)$ | $B_1 \cap B_2$                  |
| -:                   | $ Bag(t) \times Bag(t) \rightarrow Bag(t)$ | $B_1 - B_2$                     |
| symmetricDifference: | $ Bag(t) \times Bag(t) \rightarrow Bag(t)$ | $(B_1 \cup B_2)-(B_1 \cap B_2)$ |


### A.2.8 Data Signature

> **Definition A.28 (Data Signature)**
> Let $\hat{T}$ be the set of non-collection types: $\hat{T} = T_B \cup T_C$. The syntax of a data signature over an object model $M$ is a structure $\Sigma_M = (T_{M}, \Omega_{M})$ where
i. $T_{M} = T_{Expr}(\hat{T})$
ii. $\Omega_{M} =  \Omega_B \cup \Omega_C \cup  \Omega_{T_{Expr}}(\hat{T})$.
The semantics of $\Sigma_{M}$ is a structure $I(\Sigma_{M} ) = (I(T_{M}), I( \Sigma_{M} ))$ where
i. $I(T_{M})$ assigns each $t \in T_{M}$ an interpretation $I(t)$.
ii. $I(\Omega_{M} )$ assigns each operation $\omega : t_1 \times \cdot \cdot \cdot \times t_n \rightarrow t \in \Omega_{M}$ a total function $I(\omega) : I(t_1) \times \cdot\cdot\cdot \times I(t_n) \rightarrow I(t)$.
...
The core of OCL is given by an expression language. Expressions can be used in various contexts, for example, to define constraints such as class invariants. In this sub clause, we formally define the syntax and semantics of OCL expressions, and give precise meaning to notions like context, invariant.

Sub clause A.3.1 defines the abstract syntax and semantics of OCL expressions and shows how other OCL constructs can be derived from this language core. The context of expressions and invariants are discussed.
...
> iii. (a) If $t \in T_M$ and $\omega: \rightarrow t \in \Omega_M$ then $\omega \in Expr_t$, and $free (\omega ) := \emptyset$.
...
> vi. If $e_1 \in Expr_{Bag(t_1)}$, $v_1 \in Var_{t_1}$, $v_2 \in Var_{t_2}$, and $e_2, e_3 \in Expr_{t_2}$ then $e_1 \rightarrow iterate(v_1; v_2 = e_2 | e_3) \in Expr_{t_2}$ and $free(e_1 \rightarrow iterate(v_1; v_2 = e_2 | e_3)) := (free(e_1) \cup free(e_2) \cup free(e_3))-\{v1, v2 \}$.
...
& I[[ e_1 \rightarrow collectNested(v_1 | e_3) ]](r) = I[[ e_1 \rightarrow iterate(v_1; v_2 = mkBag _{type-of-e3} () | v_2 \rightarrow including(e_3) ) ]]( r)  \\
...
A system state is called valid with respect to an invariant if the invariant evaluates to true. Invariants with false result invalidate a system state.
